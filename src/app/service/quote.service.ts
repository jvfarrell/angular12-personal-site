import { Injectable } from '@angular/core';

@Injectable({
  providedIn: 'root'
})
export class QuoteService {
  quotes = [
    "What counts in life is not the mere fact that we have lived.  It is what difference we have made to the lives of others that will determine the significance of the life we lead.",
    "Hard work beats talent when talent doesn't work hard.",
    "You can't control everything. Sometimes you just need to relax and have faith that things will work out.  Let go a little and just let life happen.",
    "You can't connect the dots looking forward; you can only connect them looking backwards. So you have to trust that the dots will somehow connect in your future. You have to trust in something - your gut, destiny, life, karma, whatever. This approach has never let me down, and it has made all the difference in my life. -Steve Jobs",
    "Life is short, live it. Love is rare, grab it. Anger is bad, dump it. Fear is awful, face it. Memories are sweet, cherish it.",
    "Build your own dreams, or someone else will hire you to build theirs.",
    "The only way to do great work is to love what you do. If you haven't found it yet, keep looking. Don't settle. -  Steve Jobs",
    "Slow down, pay attention to your strengths and find out what really makes you happy",
    "It's not your strategic choices that drive success, but how well you implement them",
    "The way people treat you is a statement about who they are as a human being.       It's not a statement about you.",
    "Give her a relationship where you can act like idiots, talk about the most random stuff, share music, and never get tired of each other.",
    "I am strong because I've been weak.  I am fearless because I've been afraid.  I am wise because I've been foolish.",
    "Everyone you will ever meet knows something you don't.  -Bill Nye",
    "Go the extra mile, it's never crowded.",
    "The whole world is against me.  It wouldn't be fair otherwise.",
    "Trust me, you can dance. -Vodka",
    "VODKA.  Because no good story starts with...... that one time I ate a salad",
    "Questions can be more powerful than answers",
    "during a speech, silence can be loud....being loud can be silent...",
    "When you come to a wall that is too high to climb, throw your cap over.......then go get your cap   -Old Irish Saying",
    "So live your life that the fear of death can never enter your heart. Trouble no one about their religion;respect others in their view, and demand that they respect yours. Love your life, perfect your life, beautify all things in your life. Seek to make your life long and its purpose in the service of your people. Prepare a noble death song for the day when you go over the great divide.",
    "What you're supposed to do when you don't like a thing is change it. If you can't change it, change the way you think about it. Don't complain. - Maya Angelou, Wouldn't Take Nothing for My Journey Now",
    "Forgive, not because they deserve forgiveness.  But because you deserve peace.",
    "It is a good day to have a good day.",
    "Every cloud has a silver lining.",
    "If you wait until you can do everything for everybody, instead of something for somebody, you'll end up doing nothing for nobody.",
    "Wisdom comes not from the miles we travel along life's journey, but from the stops we make, the people we meet, and the hearts we're willing to hear.  -Larry Dobbs",
    "I think happiness is what makes you pretty. Period. Happy people are beautiful.  They become like a mirror and they reflect that happiness. -Drew Barrymore",
    "If you can't explain it simply, you don't understand it well enough. -Albert Einstein",
    "If you're dumb, surround yourself with smart people. And if you're smart, surround yourself with smart people who disagree with you. - Isaac Jaffe, Sports Night",
    "Its hard to win an argument with a smart person, but almost impossible to win one against a stupid person",
    "It is possible to commit no mistakes and still lose. That is not a weakness; that is life  -Jean-Luc Picard",
    "All we have to decide is what to do with the time that is given to us - Gandalf",
    "If more of us valued food and cheer and song above hoarded gold, it would be a merrier world. - Thorin",
    "How lucky I am to have something that makes saying goodbye so hard. -Winnie the Pooh",
    "I survived because the fire inside me burned brighter than the fire around me -Joshua Graham, The Burned Man, from Fallout: New Vegas.",
    "It does not do to dwell on dreams and forget to live - Albus Dumbledore",
    "If you look for the light, you can often find it. But if you look for the dark, that is all you will ever see.  -Uncle Iroh, The Legend of Korra",
    "Never forget what you are. The rest of the world will not. Wear it like armor, and it can never be used to hurt you. -Tyrion Lannister",
    "Nothing is really work unless you would rather be doing something else.  -J.M. Barrie",
    "The Universe is under no obligation to make sense to you.  -Neil Degrasse Tyson",
    "The work you do while you procrastinate is probably the work you should be doing for the rest of your life. -Jessica Hische",
    "Don't walk behind me; I may not lead. Don't walk in front of me; I may not follow. Just walk beside me and be my friend. - Albert Camus",
    "Courage is contagious. When a brave man takes a stand, the spines of others are often stiffened. -Billy Graham",
    "If you're not humble, life will visit humbleness upon you -Mike Tyson",
    "A man should look as if he has bought his clothes with intelligence, put them on with care and then forgotten all about them. - Hardy Amies",
    "Fairy tales are more than true-- not because they tell us dragons exist, but because they tell us dragons can be Beaten. -Neil Gaiman",
    "Power resides where men believe it resides. -Varys, Master of Whispers",
    "I carry your heart with me.... - EE Cummings",
    "I want to be your ex boyfriends stunt man.I want to do everything that he never had the courage to do like...trust you. -Rudy Francisco",
    "Work until you no longer have to introduce yourself.",
    "Sandra's seen a leprechaun, Eddie touched a troll, Laurie danced with witches once, Chalie found some goblins' gold. Donald heard a mermaid sing, Susy spied an elf, But all the magic I have known I've had to make myself.",
    "Always find a way to surround yourself with people who make you the weakest link in the chain. No matter how great you may think you are, there is alwyas someone who will make you better. By running with people faster than you, working with people stronger than you, or studying with people smarter than you, you are constantly pushed to be a better version of yourself. Mountains are constantly growing, and by staying on top of the same old hill, you will only ever be the same old you.",
    "When life gives you lemons...eat them whole. Seriously. Just choke them all down...skin, pulp, seeds, and all and don't break eye contact.  Maybe life will stop being such an asshole if you show it that you're done fucking around.",
    "Don't wait for Gandalf to show up at your door and take you on a great adventure.  Be Gandalf.",
    "One day I will find the right words and they will be simple. -Jack Kerouac",
    "Courage is grace under pressure. -Ernest Hemingway",
    "You know, sometimes all you need is twenty seconds of insane courage.  Just literally twenty seconds of just embarrassing bravery. And I promise you, something great will come of it. -Matt Damon",
    "Be the change you wish to see in the world. -Gandhi",
    "Qui sent, pleure. Qui pense, rit.  (translation: The one who feels, cries. The one who thinks, laughs.)",
    "The time you enjoy wasting is not wasted time.",
    "Death is like an old whore in a bar - I'll buy her a drink but won't go upstairs with her. -Hemingway",
    "Never regret something that once made you smile",
    "You are hanging out with us now. Dignity has nothing to do with it. -Sid Ice Age",
    "Wit is the salt of conversations, not the food.",
    "Happiness is all in your head.",
    "A kind of light spread out from her. And everything changed color. And the world opened out. And a day was good to awaken to. And there were no limits to anything. And the people of the world were good and handsome. And I was not afraid any more. - John Steinbeck, East of Eden",
    "Our employees don't really care about what we want them to do until they know how much we care about them. When an employee knows--truly knows--that you care about them, then they care about you. And when they know you care, they will listen to you and they will do anything for you",
    "No one cares how much you know until they first know how much you care about them.",
    "Love is a passion for life shared with another person. You fall in love with a person who you think is wonderful. It's your deepest appreciation of the value of that individual, and that individual is a reflection of what you value most in life. Love, for sound reasons, can be on of life's greatest rewards. -Terry Goodkind (Chainfire)",
    "Leadership is the art of getting someone else to do something you want done because they want to do it",
  ];

  getQuote() {
    var quote = this.quotes[Math.floor(Math.random()*this.quotes.length)];
    return quote;
  }

  constructor() { }
}
